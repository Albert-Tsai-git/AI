...
- 数据库：`%USERPROFILE%\sire_global\vector_db\sire_vectors.sqlite3`。
- 实现：标准库 SQLite + 预计算哈希 n-gram 向量，默认 384 维，无服务依赖。
...
```powershell
python "$env:USERPROFILE\.codex\skills\ai-dev-sire-workflow\scripts\sire_vector_db.py" search --query "任务需求" --json --limit 5 --run "$env:SIRE_RUN_DIR"
python "$env:USERPROFILE\.codex\skills\ai-dev-sire-workflow\scripts\sire_vector_db.py" show <chunk-id> --json
python "$env:USERPROFILE\.codex\skills\ai-dev-sire-workflow\scripts\sire_vector_db.py" index
python "$env:USERPROFILE\.codex\skills\ai-dev-sire-workflow\scripts\sire_vector_db.py" stats
```
